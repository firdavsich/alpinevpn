...
```bash
CID=$(docker run -d --privileged -p 1194:1194/udp -p 443:443/tcp firdavsich/alpinevpn)
...
When the `firdavsich/alpinevpn` image is started, it generates:
...
Conveniently, `firdavsich/alpinevpn` comes with a script called `serveconfig`,
